        ...
        <>
          ...
          <div className="mt-5 space-y-3">
            <div className="flex items-center justify-between gap-2 rounded-xl bg-[#f1f5fa] p-4">
              <div>
                <p className="font-archivo text-[34px] font-extrabold leading-none tracking-tight text-navy">
                  {ALIAS}
                </p>
                <p className="mt-1 text-xs text-[#475569]">{TITULAR}</p>
              </div>
              <button
                className="flex items-center gap-1.5 rounded-xl bg-navy px-4 py-3 text-sm font-bold text-white hover:opacity-90"
            ...
            <div className="flex items-center justify-between gap-2 rounded-xl bg-[#f1f5fa] p-4">
              <p className="text-[20px] font-extrabold text-tinta">{formatoPeso(monto)}</p>
              <button
                className="flex items-center gap-1.5 rounded-xl bg-navy px-4 py-3 text-sm font-bold text-white hover:opacity-90"
                onClick={() => copiar(String(Math.round(monto)), 'monto')}
              >
            ...
            <label
              className={`block cursor-pointer rounded-[14px] border-2 border-dashed p-6 text-center text-[15px] transition-colors ${
            ...
            {errorComprobante && <p className="text-center text-[15px] font-medium text-red-600">{errorComprobante}</p>}
          </div>
        </>
      ) : (
        <>
          ...
          <div className="mt-5">
            ...
            {errorMP && <p className="mt-3 text-center text-[15px] font-medium text-red-600">{errorMP}</p>}
          </div>
        </>
      )}
